...
for (model in 1:n_models){
  ...
  Landings <- c( rowSums(cp[cp$TYPE == 'PROJECTION',5:ncol(cp)]) ,
  ...
  tableg <- round(cbind(Year, OFL, Landings, Age5biomassyrs, SSB,Depl),2) %>% data.frame(.) %>% mutate("Region" = rep(mod_area, length(Year)))
  colnames(tableg) <- c('Year','Predicted OFL (mt)','ABC Catch (mt)', 'Age 5+ Biomass (mt)', 'Spawning Biomass (mt)','Depletion', 'Region')
